...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      |Shift(deltaX,deltaY,e) => 
        ...
        | Shift (dX,dY,e2) => eval_prog (Shift (dX + deltaX , dY + deltaY , e2),env)
        | _ => raise BadProgram("exp is not valid")
...
fun  preprocess_prog (e : geom_exp) : geom_exp = 
    case e of
        ...
        | Shift (deltaX,deltaY,ex) => Shift (deltaX,deltaY,preprocess_prog ex)
        | _ => e